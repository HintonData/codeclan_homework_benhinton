...
null_distribution <- msleep %>% 
  ...
  hypothesise(null = "point", mu = 7) %>%  # this time we also hypothesise
...
null_distribution_q3 <- msleep %>% 
  specify(response = sleep_total) %>%    # response = what we care about (average rating)
  hypothesise(null = "point", mu = 9.5) %>%  # this time we also hypothesise
  generate(reps = 2000, type = "bootstrap") %>%   # type will change for different tests
  calculate(stat = "mean")
```
...
null_distribution_q3 %>% 
  visualise(bins = 25) +
  shade_p_value(obs_stat = avg_herbi$mean,
...
p > alpha, therefore we may not reject the null hypothesis.
